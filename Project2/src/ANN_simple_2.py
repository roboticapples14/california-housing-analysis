...
from Models import train_neural_net
...
for (k, (train_index, test_index)) in enumerate(CV.split(X,y)): 
    ...
    mse = criterion(y_test_est, y_test)
    errors.append(mse.detach().numpy()) # store error rate for current CV fold 
...
summaries_axes[1].bar(np.arange(1, K+1), np.asarray(errors), color=color_list)
...
y_est = y_test_est.data.numpy().squeeze(1)
